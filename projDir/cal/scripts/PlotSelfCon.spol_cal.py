...
# parse the command line
    ...
    parser.add_option('--mod_rain',
                      dest='modRain', default=False,
                      action="store_true",
                      help='Plot for moderate rain')
...
def doPlot(scData, scTimes, cpData, cpTimes):
    ...
    if (options.modRain):
        biasArray = scData["dbzBiasModRain"]
    else:
    ...
    for ii, dailyBias in enumerate(dailyMeanBias):
        ...
        ax1.annotate(str,
                     ...
                     xytext=(dailyMeanTime[ii] + datetime.timedelta(0.25),  dailyBias + 0.02),
    ...
    meanBiasEarly = computePeriodStats(scTimes, biasArray, startTime, endTime)
    ...
    meanBiasLate = computePeriodStats(scTimes, biasArray, startTime, endTime)
    ...
    titleStr = "Improved self-consistency analysis - SPOL - PECAN"
    if (options.modRain):
        titleStr = titleStr + " - mod rain"
