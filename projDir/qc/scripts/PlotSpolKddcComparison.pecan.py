...
#   globals
    ...
    parser.add_option('--title',
                      dest='title',
                      default='DBZ COMPARISON BETWEEN SPOL AND KDDC - PECAN',
                      help='Title for plot')
...
def doPlot(compTimes, compData, scTimes, scData, cpTimes, cpData):
    ...
    #configureAxis(ax1, -2, 3, "SPOL minus KDDC (dB)", 'upper left')
    configureAxis(ax1, -9999, -9999, "SPOL minus KDDC (dB)", 'upper left')
    #configureAxis(ax2, -9999.0, -9999.0, "SELF CONSISTENCY BIAS KDDC (dB)", 'upper left')
    #configureAxis(ax3, 87, 88, "Xmit Power (dBM)", 'upper left')
    #configureAxis(ax3, 84, 90, "Xmit Power (dBM)", 'upper left')

    if (options.kddcAdjustDb != 0.0):
        adjustDb = str(options.kddcAdjustDb)
        #fig1.suptitle("RADAR COMPARISON SPOL-KDDC, KDDC adjusted by " + \
        #              adjustDb + " dB" + \
        #              " and SELF-CONSISTENCY CHECK ON KDDC")
        fig1.suptitle("DBZ COMPARISON SPOL-KDDC, KDDC adjusted by " + \
                      adjustDb + " dB")
    else:
        #fig1.suptitle("RADAR COMPARISON SPOL-KDDC, " + \
        #              " and SELF-CONSISTENCY CHECK ON KDDC")
        fig1.suptitle("DBZ COMPARISON SPOL-KDDC")
