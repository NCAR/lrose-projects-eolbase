...
# parse the command line
    ...
    parser.add_option('--bias_file',
                      dest='biasFilePath',
                      default='../data/pecan/zdr_bias.kgld.txt',
...
def prepareData(biasData, biasTimes):
    ...
    global tempMean, tempSdev, tempNorm, tempNorm2
    tempMean = np.mean(tempSite)
    tempSdev = np.std(tempSite)
    if (options.debug):
        print >>sys.stderr, "==>> tempMean, tempSdev: ", tempMean, tempSdev
    tempNorm = ((tempSite - tempMean) / (tempSdev * 10.0)) - 0.35
    tempNorm2 = ((tempSite - tempMean) / (tempSdev * 10.0)) - 0.25
...
def doPlot():
    ...
    title = "KGLD ZDR bias in ice and Bragg"
    ...
    ax1a.plot(btimes, tempNorm, \
              label = 'Norm-temps', color='orange', linewidth=1)
    #ax1a.plot(btimes, tempNorm2, \
    #          label = 'Norm-temps', color='orange', linewidth=1)
    
    #configDateAxis(ax1a, -9999, 9999, "ZDR Bias (dB)", 'upper right')
    configDateAxis(ax1a, -0.5, 0.1, "ZDR Bias (dB)", 'upper right')

    # ZDR vs temp

    label2a = "KGLD ZDR Bias In Ice = " + ("%.5f" % ww[0]) + " * temp + " + ("%.3f" % ww[1])
    ...
    for label2a in legend2a.get_texts():
        ...
        title2a = "KGLD ZDR Bias In Ice Vs Temp\n" + str(startTime) + " - " + str(endTime)
        #title2a = "KGLD ZDR Bias In Ice Vs Temp"
